...
- (void)viewDidLoad
{
    ...
    */
    ...
    UIBarButtonItem *signOutButton = [[UIBarButtonItem alloc] initWithTitle:@"Sign Out" style:UIBarButtonItemStyleBordered target:self action:@selector(doLogOut:)];
    ...
    axeltraLogo.backgroundColor = [UIColor clearColor];
...
-(IBAction)compare:(id)sender{
    ...
    if (appDelegate.comparison) {
        ...
        buttonsHolderFrame.origin.y = 430-146;
        ...
        CGRect tempFrame = self.view.frame;
        CGRect buttonsHolderFrame = self.buttonsHolder.frame;
        buttonsHolderFrame.origin.y = 430;
        tempFrame.size.width=320;//change acco. how much you want to expand
        tempFrame.size.height=607;
        [UIView commitAnimations];    
        [UIView animateWithDuration:0.3 animations:^(void) {
            self.scrollView.contentSize = CGSizeMake(320, 607);
            self.view.frame = tempFrame;
            self.buttonsHolder.frame = buttonsHolderFrame;
        }
...
-(IBAction)showActionSheetStartDate:(id)sender {
    ...
    [self.datePickerView setFrame:CGRectMake(0, 102, 320, 216)];
...
-(IBAction)showActionSheetEndDate:(id)sender {
    ...
    if (appDelegate.isIpad) {
        ...
        [popoverController presentPopoverFromRect:endDateButton.bounds inView:endDateButton permittedArrowDirections:UIPopoverArrowDirectionAny animated:YES];
            ...
            if([tmp count] > 0){
                ...
                if ( error == 0){
                    ...
                    if ((requestNumber == 2 && appDelegate.comparison) || (requestNumber == 1 && !appDelegate.comparison)) {
                        Costs *vievForm = [[Costs alloc] initWithNibName:@"Costs" bundle:nil];
                ...
                if ( error == 0){
                    ...
                    if((requestNumber == 2 && appDelegate.comparison) || (requestNumber == 1 && !appDelegate.comparison)){
                        if (graphView) {
                            ...
                            graphView = FALSE;
                            CPTTestAppScatterPlotController *vievForm = [[CPTTestAppScatterPlotController alloc] initWithNibName:@"ScatterPlot" bundle:nil];
                            [self.navigationController pushViewController:vievForm animated:YES];
                            [vievForm release];
                            vievForm = nil;
                        }
                        else{
                            Table *vievForm = [[Table alloc] initWithNibName:@"Table" bundle:nil];
                            [self.navigationController pushViewController:vievForm animated:YES];
                            [vievForm release];
                            vievForm = nil;
                        }

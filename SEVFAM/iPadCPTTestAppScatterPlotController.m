...
@implementation iPadCPTTestAppScatterPlotController
...
-(void)viewDidLoad
{
    ...
    [[self.navigationController view] setBounds:CGRectMake(0, 0, 748, 1024)];
    ...
    for (int i = 0; i < [niza count]; i ++) {
        double toCompare = [[[niza objectAtIndex:i] stringByReplacingOccurrencesOfString:@"," withString:@"."] doubleValue];
        if (toCompare > max) {
            max = toCompare;
        }
        if (toCompare < min) {
    ...
    if (appDelegate.comparison) {
        ...
        for (int i = 0; i < [compareNiza count]; i ++) {
            double toCompare = [[[compareNiza objectAtIndex:i] stringByReplacingOccurrencesOfString:@"," withString:@"."] doubleValue];
    ...
    axisTitleTextStyle.fontSize = 18;
    ...
    x.axisConstraints = [CPTConstraints constraintWithUpperOffset:680];
    y.axisConstraints = [CPTConstraints constraintWithUpperOffset:1010];
    ...
    if (appDelegate.comparison) {
        ...
        graph.legend.swatchSize = CGSizeMake(40.0, 40.0);
        graph.legendAnchor = CPTRectAnchorRight;
        graph.legendDisplacement = CGPointMake(-5.0, 320.0);
